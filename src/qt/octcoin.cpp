...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<OctcoinUnit>("OctcoinUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. octcoin_de.qm (shortcut "de" needs to be defined in octcoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. octcoin_de_DE.qm (shortcut "de_DE" needs to be defined in octcoin.qrc)
...
static const char* qt_argv = "octcoin-qt";

OctcoinApplication::OctcoinApplication()
...
void OctcoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the OctcoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", OctcoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void OctcoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool OctcoinApplication::createOptionsModel(bool resetSettings)
{
...
void OctcoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new OctcoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &OctcoinGUI::quitRequested, this, &OctcoinApplication::requestShutdown);
...
void OctcoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void OctcoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool OctcoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void OctcoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &OctcoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &OctcoinApplication::handleRunawayException);
    connect(this, &OctcoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &OctcoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void OctcoinApplication::parameterSetup()
{
...
void OctcoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void OctcoinApplication::requestInitialize()
{
...
void OctcoinApplication::requestShutdown()
{
...
void OctcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // octcoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &OctcoinGUI::handlePaymentRequest);
            connect(window, &OctcoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void OctcoinApplication::handleRunawayException(const QString &message)
{
...
void OctcoinApplication::handleNonFatalException(const QString& message)
{
...
WId OctcoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool OctcoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", OctcoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(octcoin);
    Q_INIT_RESOURCE(octcoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(OCTCOIN_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see octcoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see octcoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse octcoin.conf, determine network, switch to network specific
    ...
    // octcoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
