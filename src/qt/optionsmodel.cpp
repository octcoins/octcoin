...
/** Call node.updateRwSetting() with Octcoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Octcoin 22.x releases try to read these specific settings as
        ...
        // in later releases by  https://github.com/octcoins/octcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because octcoin 22.x will not try to read these.
...
//! Convert enabled/size values to octcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from octcoin -prune setting.
...
//! Get pruning size value to show in GUI from octcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayOctcoinUnit")) {
        settings.setValue("DisplayOctcoinUnit", QVariant::fromValue(OctcoinUnit::OTC));
    }
    QVariant unit = settings.value("DisplayOctcoinUnit");
    if (unit.canConvert<OctcoinUnit>()) {
        m_display_octcoin_unit = unit.value<OctcoinUnit>();
    } else {
        m_display_octcoin_unit = OctcoinUnit::OTC;
        settings.setValue("DisplayOctcoinUnit", QVariant::fromValue(m_display_octcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if octcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_octcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<OctcoinUnit>() == m_display_octcoin_unit) return;
    m_display_octcoin_unit = new_unit.value<OctcoinUnit>();
    QSettings settings;
    settings.setValue("DisplayOctcoinUnit", QVariant::fromValue(m_display_octcoin_unit));
    Q_EMIT displayUnitChanged(m_display_octcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see  https://github.com/octcoins/octcoin/pull/8273
    ...
    // (https://github.com/octcoin-core/gui/issues/567).
