...
#include <octcoin-build-config.h> // IWYU pragma: keep
#include <qt/miningpage.h>
#include <qt/octcoingui.h>
...
const std::string OctcoinGUI::DEFAULT_UIPLATFORM =
...
OctcoinGUI::OctcoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &OctcoinGUI::createWallet);
        connect(walletFrame, &WalletFrame::message, [this](const QString& title, const QString& message, unsigned int style) {
            this->message(title, message, style);
        });
        connect(walletFrame, &WalletFrame::currentWalletSet, [this] { updateWalletStatus(); });
        // Stacked widget pour naviguer entre walletFrame et miningPage
        stackedWidget = new QStackedWidget(this);
        stackedWidget->addWidget(walletFrame);
        miningPage = new MiningPage(this);
        stackedWidget->addWidget(miningPage);
        stackedWidget->setCurrentWidget(walletFrame);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &OctcoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &OctcoinGUI::showModalOverlay);
...
void OctcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Octcoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and octcoin: URIs)"));
    ...
    miningAction->setStatusTip(tr("Mine Octcoin with your CPU"));
    ...
    connect(overviewAction, &QAction::triggered, this, &OctcoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &OctcoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &OctcoinGUI::gotoHistoryPage);
    connect(miningAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(miningAction, &QAction::triggered, this, &OctcoinGUI::gotoMiningPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Octcoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Octcoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Octcoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Octcoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a octcoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Octcoin command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &OctcoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &OctcoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &OctcoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &OctcoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &OctcoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &OctcoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &OctcoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &OctcoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &OctcoinGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &OctcoinGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &OctcoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &OctcoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &OctcoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &OctcoinGUI::showDebugWindow);
}

void OctcoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void OctcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &OctcoinGUI::setCurrentWalletBySelectorIndex);
...
void OctcoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &OctcoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &OctcoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &OctcoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &OctcoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void OctcoinGUI::enableHistoryAction(bool privacy)
{
...
void OctcoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &OctcoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &OctcoinGUI::removeWallet);
...
WalletController* OctcoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void OctcoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &OctcoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &OctcoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &OctcoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &OctcoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &OctcoinGUI::incomingTransaction);
    connect(this, &OctcoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void OctcoinGUI::removeWallet(WalletModel* walletModel)
{
...
void OctcoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void OctcoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void OctcoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void OctcoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void OctcoinGUI::createTrayIcon()
{
...
void OctcoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &OctcoinGUI::toggleHidden);
...
void OctcoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void OctcoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void OctcoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void OctcoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void OctcoinGUI::showHelpMessageClicked()
{
...
void OctcoinGUI::gotoMiningPage()
{
    miningAction->setChecked(true);
    if (stackedWidget && miningPage)
        stackedWidget->setCurrentWidget(miningPage);
}

void OctcoinGUI::openClicked()
{
...
void OctcoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (stackedWidget && walletFrame)
        stackedWidget->setCurrentWidget(walletFrame);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void OctcoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void OctcoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void OctcoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void OctcoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void OctcoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void OctcoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void OctcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Octcoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void OctcoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void OctcoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void OctcoinGUI::updateHeadersSyncProgressLabel()
{
...
void OctcoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void OctcoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &OctcoinGUI::quitRequested);
...
void OctcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void OctcoinGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &OctcoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void OctcoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void OctcoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &OctcoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &OctcoinGUI::show);
                e->ignore();
            }
        }
    }
...
void OctcoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void OctcoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void OctcoinGUI::incomingTransaction(const QString& date, OctcoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(OctcoinUnits::formatWithUnit(unit, amount, true));
...
void OctcoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void OctcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool OctcoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool OctcoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void OctcoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void OctcoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void OctcoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void OctcoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void OctcoinGUI::updateWindowTitle()
{
...
void OctcoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void OctcoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void OctcoinGUI::detectShutdown()
{
...
void OctcoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void OctcoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(OctcoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = OctcoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void OctcoinGUI::subscribeToCoreSignals()
{
...
void OctcoinGUI::unsubscribeFromCoreSignals()
{
...
bool OctcoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<OctcoinUnit> units = OctcoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const OctcoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, OctcoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const OctcoinUnit u : OctcoinUnits::availableUnits()) {
        menu->addAction(OctcoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(OctcoinUnit newUnits)
{
    setText(OctcoinUnits::longName(newUnits));
}
