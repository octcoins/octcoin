...
// Hashrate reel du mineur CPU (generatecontinuous)
static std::atomic<int64_t> g_cpu_hashrate{0};
...
static RPCHelpMan generatetodescriptor()
{
    return RPCHelpMan{
        ...
        "Mine to a specified descriptor and return the block hashes.",
        {
            ...
            {"descriptor", RPCArg::Type::STR, RPCArg::Optional::NO, "The descriptor to send the newly generated octcoin to."},
        ...
        "Mine to a specified address and return the block hashes.",
         {
             ...
             {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated octcoin to."},
             {"maxtries", RPCArg::Type::NUM, RPCArg::Default{DEFAULT_MAX_TRIES}, "How many iterations to try."},
         },
         RPCResult{
             RPCResult::Type::ARR, "", "hashes of blocks generated",
         ...
         RPCExamples{
            ...
            + "If you are using the " CLIENT_NAME " wallet, you can get a new address to send the newly generated octcoin to with:\n"
            + HelpExampleCli("getnewaddress", "")
                },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    const int num_blocks = request.params[0].isNum() ? request.params[0].getInt<int>() : std::stoi(request.params[0].get_str());
...
static RPCHelpMan generateblock()
{
    return RPCHelpMan{"generateblock",
        ...
        {
            {"output", RPCArg::Type::STR, RPCArg::Optional::NO, "The address or descriptor to send the newly generated octcoin to."},
        ...
        "Returns a json object containing mining-related information.",
                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        ...
                        {RPCResult::Type::NUM, "cpuhashps", "CPU miner hashrate (generatecontinuous)"},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    obj.pushKV("cpuhashps", g_cpu_hashrate.load());
...
// NOTE: Unlike wallet RPC (which use OTC values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
static RPCHelpMan getblocktemplate()
{
    return RPCHelpMan{
        ...
        "    https://github.com/octcoin/bips/blob/master/bip-0022.mediawiki\n"
        "    https://github.com/octcoin/bips/blob/master/bip-0023.mediawiki\n"
        "    https://github.com/octcoin/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
        "    https://github.com/octcoin/bips/blob/master/bip-0145.mediawiki\n",
        {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!lpval.isNull()) {
         ...
         *
         * getblocktemplate is unlikely to be called by octcoin-cli, so
...
static RPCHelpMan submitblock()
{
    ...
    return RPCHelpMan{
        ...
        "See https://en.octcoin.it/wiki/BIP_0022 for full specification.\n",
        {
...
static RPCHelpMan generatecontinuous()
{
    return RPCHelpMan{"generatecontinuous",
        "Mine blocks continuously until stopped. Each block reward goes to a new address (like Satoshi).",
        {
            {"maxtries", RPCArg::Type::NUM, RPCArg::Default{DEFAULT_MAX_TRIES}, "Max iterations per block."},
        },
        RPCResult{RPCResult::Type::NUM, "", "Total blocks mined."},
        RPCExamples{HelpExampleCli("generatecontinuous", "")},
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    const int nblocks{request.params[0].isNull() ? 0 : request.params[0].getInt<int>()};
    const uint64_t max_tries{request.params[1].isNull() ? DEFAULT_MAX_TRIES : request.params[1].getInt<uint64_t>()};

    NodeContext& node = EnsureAnyNodeContext(request.context);
    Mining& miner = EnsureMining(node);
    ChainstateManager& chainman = EnsureChainman(node);

    // Get wallet from wallet_loader
    if (!node.wallet_loader) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Wallet not enabled");
    }
    ...
    // Find the wallet requested via -rpcwallet or the only loaded wallet
    std::shared_ptr<wallet::CWallet> pwallet;
    {
        LOCK(wallet_context->wallets_mutex);
        if (wallet_context->wallets.empty()) {
            throw JSONRPCError(RPC_WALLET_ERROR, "No wallet loaded");
        }
        // Use the first wallet if only one, otherwise check rpcwallet header
        // Try to match by URI path /wallet/<name>
        std::string uri_wallet;
        if (request.URI.size() > 8 && request.URI.substr(0, 8) == "/wallet/") {
            uri_wallet = UrlDecode(std::string_view{request.URI}.substr(8));
        }
        if (!uri_wallet.empty()) {
            for (auto& w : wallet_context->wallets) {
                if (w->GetName() == uri_wallet) { pwallet = w; break; }
            }
            if (!pwallet) throw JSONRPCError(RPC_WALLET_NOT_FOUND, "Wallet not found: " + uri_wallet);
        } else {
            if (wallet_context->wallets.size() > 1) {
                throw JSONRPCError(RPC_WALLET_ERROR, "Multiple wallets loaded. Use -rpcwallet=<name>");
            }
            pwallet = wallet_context->wallets[0];
        }
    }

    uint64_t total_blocks = 0;

    while (!chainman.m_interrupt && (nblocks == 0 || (int64_t)total_blocks < nblocks)) {
        CTxDestination dest;
        {
            LOCK(pwallet->cs_wallet);
            auto op_dest = pwallet->GetNewDestination(pwallet->m_default_address_type, "mining");
        ...
        uint32_t nonce_start = block.nNonce;
        while (!chainman.m_interrupt && !found) {
            while (block.nNonce < std::numeric_limits<uint32_t>::max() && !chainman.m_interrupt) {
                if (CheckProofOfWork(block.GetHash(), block.nBits, chainman.GetConsensus())) {
                    found = true;
                    break;
                }
                ++block.nNonce;
                if (block.nNonce % 150000 == 0) {
                    auto elapsed = Ticks<MillisecondsDouble>(SteadyClock::now() - t_start);
                    uint32_t hashes = block.nNonce - nonce_start;
                    g_cpu_hashrate.store(elapsed > 0 ? (int64_t)(hashes * 1000.0 / elapsed) : 0);
                }
            }
            if (!found && !chainman.m_interrupt) {
                block.nTime = static_cast<uint32_t>(TicksSinceEpoch<std::chrono::seconds>(NodeClock::now()));
                block.nNonce = 0;
            }
        }
        if (found && !chainman.m_interrupt) {
            auto shared_block = std::make_shared<const CBlock>(block);
            if (chainman.ProcessNewBlock(shared_block, true, true, nullptr)) {
                total_blocks++;
                LogInfo("generatecontinuous: mined block #%d hash=%s",
                        total_blocks, shared_block->GetHash().GetHex());
            }
        }
    }
    return UniValue(total_blocks);
},
    };
}
