...
#ifndef OCTCOIN_HASH_H
#define OCTCOIN_HASH_H
...
/** A hasher class for Octcoin's 256-bit hash (double SHA-256). */
class CHash256 {
private:
    CSHA256 sha;
public:
...
/** A hasher class for Octcoin's 160-bit hash (SHA-256 + RIPEMD-160). */
class CHash160 {
private:
    CSHA256 sha;
public:
...
/** A writer stream (for serialization) that computes a 512-bit double hash (SHA512d) truncated to 256 bits. */
class HashWriter512
{
private:
    CSHA512 ctx;
...
public:
    ...
    uint256 GetHash() {
        ...
        ctx.Finalize(buf);
        uint256 result;
        memcpy(result.begin(), buf, 32);
        return result;
    }

    template <typename T>
    HashWriter512& operator<<(const T& obj) {
        ::Serialize(*this, obj);
        return *this;
    }
};

#endif // OCTCOIN_HASH_H
