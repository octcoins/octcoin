...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking octcoin code so they have to run
    ...
    // functions run, so client objects are fully disconnected before octcoin
    // code in the destructors are run. This way if the octcoin code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In octcoin core, running these callbacks in the right order is
