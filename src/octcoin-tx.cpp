...
static void SetupOctcoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " octcoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The octcoin-tx tool is used for creating and modifying octcoin transactions.\n\n"
                "octcoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded octcoin transaction, or with \"-create [commands]\" to create a hex-encoded octcoin transaction.\n"
                "\n"
                "Usage: octcoin-tx [options] <hex-tx> [commands]\n"
                "or:    octcoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded octcoin transaction
