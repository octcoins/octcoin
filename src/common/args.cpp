...
const char * const OCTCOIN_CONF_FILENAME = "octcoin.conf";
const char * const OCTCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //octcoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", OCTCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Octcoin
    //   new: C:\Users\Username\AppData\Local\Octcoin
    // macOS: ~/Library/Application Support/Octcoin
    // Unix-like: ~/.octcoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Octcoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Octcoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Octcoin";
#else
    // Unix-like
    return pathRet / ".octcoin";
#endif
#endif
}
