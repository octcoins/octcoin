// Copyright (c) 2024 The Octcoin Core developers
...
std::optional<CNetAddr> QueryDefaultGatewayImpl(sa_family_t family)
{
    ...
    if (status != NO_ERROR) {
        // Fallback: enumerate interfaces and pick first non-loopback
        PMIB_IPADDRTABLE pIPAddrTable = nullptr;
        DWORD dwSize = 0;
        GetIpAddrTable(pIPAddrTable, &dwSize, 0);
        pIPAddrTable = (MIB_IPADDRTABLE*)malloc(dwSize);
        if (pIPAddrTable && GetIpAddrTable(pIPAddrTable, &dwSize, 0) == NO_ERROR) {
            for (DWORD i = 0; i < pIPAddrTable->dwNumEntries; i++) {
                DWORD addr = pIPAddrTable->table[i].dwAddr;
                if ((addr & 0xFF) != 127) {
                    best_if_idx = pIPAddrTable->table[i].dwIndex;
                    break;
                }
            }
        }
        free(pIPAddrTable);
        if (best_if_idx == 0) {
            LogPrintLevel(BCLog::NET, BCLog::Level::Debug, "Could not get best interface for default route: %s\n", NetworkErrorString(status));
            return std::nullopt;
        }
    }
    ...
    if (status != NO_ERROR) {
        LogPrintLevel(BCLog::NET, BCLog::Level::Debug, "Could not get best route for default route for interface index %d: %s\n",
