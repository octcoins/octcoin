...
Options:
  -m, --multiprocess     Run multiprocess binaries octcoin-node, octcoin-gui.
  -M, --monolithic       Run monolithic binaries octcoind, octcoin-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'octcoin-qt [ARGS]' or 'octcoin-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'octcoind [ARGS]' or 'octcoin-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'octcoin-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'octcoin-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'octcoin-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_octcoin [ARGS]'.
  chainstate [ARGS] Run octcoin kernel chainstate util, equivalent to running 'octcoin-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_octcoin [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_octcoin-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(cmd.use_multiprocess ? "octcoin-gui" : "octcoin-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(cmd.use_multiprocess ? "octcoin-node" : "octcoind");
        } else if (cmd.command == "rpc") {
            args.emplace_back("octcoin-cli");
            // Since "octcoin rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("octcoin-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("octcoin-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_octcoin");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("octcoin-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_octcoin");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_octcoin-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("octcoin-util");
        } else {
...
//! Execute the specified octcoind, octcoin-qt or other command line in `args`
...
//!             be a relative path to a octcoind, octcoin-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/octcoin ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // ( https://github.com/octcoins/octcoin/pull/31375#discussion_r1861814807)
