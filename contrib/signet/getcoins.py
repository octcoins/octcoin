#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Octcoin Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing octcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='octcoin-cli', help='octcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Octcoin address to which the faucet should send')
...
parser.add_argument('octcoin_cli_args', nargs='*', help='Arguments to pass on to octcoin-cli (default: -signet)')

args = parser.parse_args()

if args.octcoin_cli_args == []:
    args.octcoin_cli_args = ['-signet']


def octcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.octcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = octcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = octcoin_cli(['getnewaddress', 'faucet', 'bech32'])
