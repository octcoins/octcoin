# bash programmable completion for octcoin-cli(1)
# Copyright (c) 2012-2022 The Octcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $octcoin-cli for RPC
_octcoin_rpc() {
...
_octcoin_cli() {
    ...
    # save and use original argument to invoke octcoin-cli for -help, help and RPC
    # as octcoin-cli might not be in $PATH
    octcoin_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($octcoin_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_octcoin_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _octcoin_cli octcoin-cli
