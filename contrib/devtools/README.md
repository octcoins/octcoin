...
Provides utilities for managing copyright headers of `The Octcoin Core
...
Generates a octcoin.conf file in `share/examples/` by parsing the output from `octcoind --help`. This script is run during the
release process to include a octcoin.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-octcoin-conf.sh
```
