...
"""Script for verifying Octcoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from octcoincore.org and octcoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/octcoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://octcoincore.org"
HOST2 = "https://octcoin.org"
VERSIONPREFIX = "octcoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Octcoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"octcoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by octcoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the octcoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes octcoin.org lags behind octcoincore.org.)')
    )
