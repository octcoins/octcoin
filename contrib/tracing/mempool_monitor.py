#!/usr/bin/env python3
# Copyright (c) 2022 The Octcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

""" Example logging Octcoin Core mempool events using the mempool:added,
...
def main(pid):
    print(f"Hooking into octcoind with pid {pid}")
    octcoind_with_usdts = USDT(pid=int(pid))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    octcoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    octcoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    octcoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    octcoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[octcoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "<pid of octcoind>")
        exit(1)
