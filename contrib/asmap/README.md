...
be used in Octcoin Core with the `-asmap` option.
...
to binary before being used with Octcoin Core.
...
the `octcoin-cli getnodeaddresses` command.
...
```
octcoin-cli getnodeaddresses 0 > addrs.json
```
