#!/usr/bin/env bash
# Copyright (c) 2019-2022 The Octcoin Core developers
...
(
    ...
    case "$HOST" in
        *mingw*)
            ...
            mv build/octcoin-win64-setup.exe "${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Octcoin Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Octcoin Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example octcoin.conf file. if contrib/devtools/gen-octcoin-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/octcoin.conf" "${DISTNAME}/"
    ...
    case "$HOST" in
        ...
        *darwin*)
            ...
            mv build/dist/Octcoin-Core.zip "${OUTDIR}/${DISTNAME}-${HOST}-unsigned.zip"
