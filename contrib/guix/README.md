# Bootstrappable Octcoin Core Builds

This directory contains the files necessary to perform bootstrappable Octcoin
Core builds.
...
To build Octcoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Octcoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/octcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/octcoin-core/guix.sigs).
