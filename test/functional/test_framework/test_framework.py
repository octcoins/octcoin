#!/usr/bin/env python3
# Copyright (c) 2014-present The Octcoin Core developers
...
TMPDIR_PREFIX = "octcoin_func_test_"
...
class Binaries:
    """Helper class to provide information about octcoin binaries
    ...
    def node_argv(self, **kwargs):
        "Return argv array that should be used to invoke octcoind"
        return self._argv("node", self.paths.octcoind, **kwargs)

    def rpc_argv(self):
        "Return argv array that should be used to invoke octcoin-cli"
        # Add -nonamed because "octcoin rpc" enables -named by default, but octcoin-cli doesn't
        return self._argv("rpc", self.paths.octcoincli) + ["-nonamed"]

    def tx_argv(self):
        "Return argv array that should be used to invoke octcoin-tx"
        return self._argv("tx", self.paths.octcointx)

    def util_argv(self):
        "Return argv array that should be used to invoke octcoin-util"
        return self._argv("util", self.paths.octcoinutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke octcoin-wallet"
        return self._argv("wallet", self.paths.octcoinwallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke octcoin-chainstate"
        return self._argv("chainstate", self.paths.octcoinchainstate)

    def _argv(self, command, bin_path, need_ipc=False):
        """Return argv array that should be used to invoke the command. It
        either uses the octcoin wrapper executable (if OCTCOIN_CMD is set or
        need_ipc is True), or the direct binary path (octcoind, etc). When
        bin_dir is set (by tests calling binaries from previous releases) it
        always uses the direct path."""
        if self.bin_dir is not None:
            return [os.path.join(self.bin_dir, os.path.basename(bin_path))]
        elif self.paths.octcoin_cmd is not None or need_ipc:
            # If the current test needs IPC functionality, use the octcoin
            # wrapper binary and append -m so it calls multiprocess binaries.
            octcoin_cmd = self.paths.octcoin_cmd or [self.paths.octcoin_bin]
            return octcoin_cmd + (["-m"] if need_ipc else []) + [command]
        else:
            return [bin_path]


class OctcoinTestMetaClass(type):
    """Metaclass for OctcoinTestFramework.

    Ensures that any attempt to register a subclass of `OctcoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'OctcoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("OctcoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("OctcoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class OctcoinTestFramework(metaclass=OctcoinTestMetaClass):
    """Base class for a octcoin test script.

    Individual octcoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave octcoinds and test.* datadir on exit or error")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use octcoin-cli instead of RPC for all commands")
    ...
    def get_binary_paths(self):
        ...
        binaries = {
            "octcoin": "OCTCOIN_BIN",
            "octcoind": "OCTCOIND",
            "octcoin-cli": "OCTCOINCLI",
            "octcoin-util": "OCTCOINUTIL",
            "octcoin-tx": "OCTCOINTX",
            "octcoin-chainstate": "OCTCOINCHAINSTATE",
            "octcoin-wallet": "OCTCOINWALLET",
        }
        # Set paths to octcoin core binaries allowing overrides with environment
        # variables.
        ...
        # OCTCOIN_CMD environment variable can be specified to invoke octcoin
        # wrapper binary instead of other executables.
        paths.octcoin_cmd = shlex.split(os.getenv("OCTCOIN_CMD", "")) or None
        return paths
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("OctcoinRPC")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a octcoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple octcoind test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the octcoind
    ...
    def _start_logging(self):
        ...
        # Format logs the same as octcoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in octcoin.conf
    ...
    def skip_if_no_octcoind_tracepoints(self):
        """Skip the running test if octcoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("octcoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_octcoind_zmq(self):
        """Skip the running test if octcoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("octcoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if octcoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("octcoin-wallet has not been compiled")

    def skip_if_no_octcoin_tx(self):
        """Skip the running test if octcoin-tx has not been compiled."""
        if not self.is_octcoin_tx_compiled():
            raise SkipTest("octcoin-tx has not been compiled")

    def skip_if_no_octcoin_util(self):
        """Skip the running test if octcoin-util has not been compiled."""
        if not self.is_octcoin_util_compiled():
            raise SkipTest("octcoin-util has not been compiled")

    def skip_if_no_octcoin_chainstate(self):
        """Skip the running test if octcoin-chainstate has not been compiled."""
        if not self.is_octcoin_chainstate_compiled():
            raise SkipTest("octcoin-chainstate has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if octcoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("octcoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether octcoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether octcoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_octcoin_tx_compiled(self):
        """Checks whether octcoin-tx was compiled."""
        return self.config["components"].getboolean("BUILD_OCTCOIN_TX")

    def is_octcoin_util_compiled(self):
        """Checks whether octcoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_OCTCOIN_UTIL")

    def is_octcoin_chainstate_compiled(self):
        """Checks whether octcoin-chainstate was compiled."""
        return self.config["components"].getboolean("ENABLE_OCTCOIN_CHAINSTATE")
