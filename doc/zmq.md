...
The Octcoin Core daemon can be configured to act as a trusted "border
router", implementing the octcoin wire protocol and relay, making
...
The ZeroMQ feature in Octcoin Core requires the ZeroMQ API >= 4.0.0
...
For instance:
    ...
    $ octcoind -zmqpubhashtx=tcp://127.0.0.1:28554 \
               -zmqpubhashtx=tcp://192.168.1.2:28554 \
               -zmqpubhashblock="tcp://[::1]:28555" \
               -zmqpubrawtx=unix:/tmp/octcoind.tx.raw \
...
ZMQ_SUBSERIBE option set to one or either of these prefixes (for
...
From the perspective of octcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into octcoind directly. Furthermore, no information is
...
using. Octcoind appends an up-counting sequence number to each
