# CJDNS support in Octcoin Core

It is possible to run Octcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Octcoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Octcoin network and individual nodes.
...
before it will work with your Octcoin Core node. This is described in steps
...
## Run Octcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Octcoin Core
...
When enabled, this option tells Octcoin Core that it is running in an
...
IPv6 local network. This helps Octcoin Core perform better address management:
...
CJDNS support was added to Octcoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `octcoin-cli -addrinfo` to see the
...
[doc/tor.md]( https://github.com/octcoins/octcoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Octcoin Core

There are several ways to see your CJDNS address in Octcoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `octcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `octcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `octcoin-cli help getnodeaddresses` for details.
