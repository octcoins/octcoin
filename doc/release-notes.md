...
Octcoin Core version v30.0 is now available from:

  <https://octcoins.github.io/bin/octcoin-core-30.0/>
...
Octcoin Core is supported and tested on operating systems using the
Linux Kernel 3.17+, macOS 13+, and Windows 10+. Octcoin
...
- A new `octcoin` command line tool has been added to make features more discoverable
  and convenient to use. The `octcoin` tool just calls other executables and does not
  implement any functionality on its own. Specifically `octcoin node` is a synonym for
  `octcoind`, `octcoin gui` is a synonym for `octcoin-qt`, and `octcoin rpc` is a synonym
  for `octcoin-cli -named`. Other commands and options can be listed with `octcoin help`.
  The new `octcoin` command is an alternative to calling other commands directly, but it
...
- The new `octcoin` command does support one new feature: an (experimental) IPC Mining
  Interface that allows the node to work with Stratum v2 or other mining client software,
  see (#31098). When the node is started with `octcoin -m node -ipcbind=unix` it will
  ...
  (`octcoin-node` instead of `octcoind`) and is currently required but will become optional
  in the future (with [#33229]( https://github.com/octcoins/octcoin/pull/33229)).

- IPC connectivity introduces new dependencies (see [multiprocess.md]( https://github.com/octcoins/octcoin/blob/master/doc/multiprocess.md)),
...
- The `test_octcoin` executable is now installed in `libexec/` instead of `bin/`.
  It can still be executed directly, or accessed through the new `octcoin` command
  as `octcoin test`. The `libexec/` directory also contains new `octcoin-node` and
  `octcoin-gui` binaries which support IPC features and are called through the
  `octcoin` tool. In source builds only, `test_octcoin-qt`, `bench_octcoin`, and
  `octcoin-chainstate` are also now installed to `libexec/` instead of `bin/` and
  can be accessed through the new `octcoin` command. See `octcoin help` output for
  details. (#31679)
...
- The implementation of coinstatsindex was changed to prevent an overflow bug that
  ...
  from their datadir. A future release of Octcoin Core may remove the old version of the
...
- The `-paytxfee` startup option and the `settxfee` RPC are now deprecated and
  will be removed in Octcoin Core 31.0. They allowed the user to set a static fee
...
- The legacy wallet removal drops redundant options in the octcoin-wallet tool,
...
[Transifex](https://explore.transifex.com/octcoin/octcoin/).
