Octcoin Core version 28.0 is now available from:

  <https://octcoins.github.io/bin/octcoin-core-28.0/>
...
Support for Testnet4 as specified in [BIP94](https://github.com/octcoin/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\Octcoin`
to `C:\Users\Username\AppData\Local\Octcoin`. Octcoin Core will check the existence
...
See [JSON-RPC-interface.md]( https://github.com/octcoins/octcoin/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
liboctcoinconsensus Removal
---------------------------

The liboctcoin-consensus library was deprecated in 27.0 and is now completely removed. (#29648)

P2P and Network Changes
-----------------------

- Previously if Octcoin Core was listening for P2P connections, either using
  default settings or via `bind=addr:port` it would always also bind to
  `127.0.0.1:6556` to listen for Tor connections. It was not possible to switch
  ...
  of binding to `0.0.0.0:8555` and `127.0.0.1:6556` has not been changed.
  ...
  `127.0.0.1:6556`, you need to now make this explicit by using
  `bind=... bind=127.0.0.1:6556=onion`. (#22729)

- Octcoin Core will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/octcoin/bips/blob/master/bip-0431.mediawiki).  The
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running Octcoin Core with the configuration
...
are now required to compile Octcoin Core. (#29091, #30263)

- The minimum required glibc to run Octcoin Core is now
...
- A new `-testdatadir` option has been added to `test_octcoin` to allow specifying the
...
Previous releases of Octcoin Core or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
